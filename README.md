...
- **地点感知事件导演**：白桦站事件采用 `dormant → eligible → queued → resolved/expired` 生命周期。事件越过时间点后先进入可触发状态，再根据玩家当前位置、明确移动意图和地点邻接关系选择现场介入、附近拦截、延后或远程后果；远程事件立即给出烟雾、断电、异响等可感知变化，幕后身份仍留待玩家调查。
...
- **模型容错事件**：模型必须用 `active_event_ack` 确认已把当前事件写入叙事。错配或遗漏会在三次请求预算内重试；若模型始终不合规，引擎会先显示作者编写的中文场内线索，再结算事件，不会静默丢失。
- **NPC 信息与终局**：普通界面只显示主角的精确 HP/SAN；可见 NPC 显示伤势与精神状态的中文定性描述。06:00 后时钟冻结，先解决当前危险，再由玩家从四个服务端终局选项中明确决定；结局必须收束冲突、主角、相关角色与真相去向，不完整输出会重试并在必要时由引擎生成确定性收束。
...
LLM 未按要求输出合法 JSON 或未通过事件/结局语义校验时，系统最多发起三次请求，随后按流程使用作者线索或状态化结局兜底。
...
SoCLaaS 提供 OpenAI-compatible 的 `/v1/chat/completions` 接口，可以直接复用本项目后端的 LLM provider。不要把 API Key 放进前端代码；只配置在 `backend/.env`：
...
调查从 00:10 开始。06:00 到达后进入终局门：时钟冻结，玩家可在不再消耗游戏时间的情况下先解决正在发生的危险，再决定真相与证据的最终去向。每次有实际意义的普通调查行动至少推进十分钟；随着时间流逝，计划事件会改变可获得的线索、风险与可选行动。
...
- **状态驱动结局：** HP/SAN 归零会立即触发死亡或疯狂结局；06:00 只打开终局门，不会跳过未完成的危险或替玩家选择。LLM 只负责书写通过完整性校验的结局，而不决定结局是否可触发。
