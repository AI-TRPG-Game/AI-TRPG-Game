...
export const BIRCH_STATION_TUTORIAL = {
  ...
  scenarioRules: {
    ...
    clueCatalog: {
      ...
      evidence_002: { category: 'murder', source: '地毯湿泥', reliability: 'medium', description: '地毯上的湿泥来自关闭的车顶通道。', discoveryHint: '在包厢门口或行李车检查湿泥的颗粒与来源。', preservationHint: '将泥粒装袋取样，并记录发现位置。', locationId: 'loc_001', keywords: ['湿泥', '泥', '地毯', '脚印', '泥粒'], truths: ['murder'] },
      evidence_003: { category: 'murder', source: '针孔与医疗记录', reliability: 'high', description: '死因被伪装成急病，实际与镇静剂注射有关。', discoveryHint: '检查尸体手臂针孔，并把它和医疗档案室的记录互相核对。', preservationHint: '拍摄针孔并抄录对应药物记录的日期和签名。', locationId: 'loc_007', locationIds: ['loc_001', 'loc_007'], keywords: ['针孔', '针眼', '镇静剂', '注射', '死因', '药物记录'], truths: ['murder', 'culprit'] },
      evidence_004: { category: 'survivor', source: '顾言的录音', reliability: 'high', description: '失踪的录音提到矿难后仍有第七名幸存者。', discoveryHint: '修复或完整听取顾言留下的录音，记录“第七个人”的原话。', preservationHint: '复制录音内容，并逐字抄录关键原话。', locationId: 'loc_001', keywords: ['录音', '录音带', '第七个人', '完整听'], truths: ['seventh_survivor'] },
      evidence_005: { category: 'coverup', source: '调度记录', reliability: 'high', description: '调度日志被改写，用来掩盖一次未登记的转运。', discoveryHint: '在站务办公室找到原始调度记录，或在程岳销毁前抢救残页。', preservationHint: '带走原始记录或残页，并拍摄完整页码和改写痕迹。', locationId: 'loc_006', keywords: ['调度', '日志', '残页', '转运记录'], truths: ['coverup', 'culprit'] },
      evidence_006: { category: 'survivor', source: '苏棠的画与乘客名册', reliability: 'high', description: '画作和名册共同证明第七名幸存者从官方人数中被抹掉。', discoveryHint: '询问苏棠并比对她的画与乘客名册，不要只凭口述。', preservationHint: '分别拍摄画作和名册，并记录二者可相互印证的位置。', locationId: 'loc_005', keywords: ['苏棠', '画', '画作', '名册', '第七名'], truths: ['seventh_survivor'] },
      evidence_007: { category: 'coverup', source: '医疗转运档案', reliability: 'high', description: '医疗文件把矿难幸存者与白桦站的转运联系起来。', discoveryHint: '在医疗档案室寻找封蜡病历和转运表，注意日期与签名。', preservationHint: '封存或带走关键档案，并抄录日期、签名和转运编号。', locationId: 'loc_007', keywords: ['医疗', '档案', '病历', '转运表', '封蜡'], truths: ['coverup'] },
      evidence_008: { category: 'culprit', source: '林晚的证词', reliability: 'high', description: '证词把共谋者、篡改记录和死者放在同一条行动链上。', discoveryHint: '先取得林晚信任，再让她在安全地点交出检修图并说明见闻。', preservationHint: '在征得林晚同意后录音或形成签字证词，并安全保存检修图。', locationId: 'loc_008', keywords: ['林晚', '检修图', '证词', '见闻', '检修班表'], truths: ['culprit'] },
    },
    truths: {
...
你可以点击下方选项，也可以自由输入。本次只能选择一种：**移动到一个地点，或在当前地点执行一个有意义的行动**，不能把移动和到达后的调查一次做完。例如，先输入“进入行李车”，下一次再输入“检查行李车里的箱子”，不要合写成“去行李车并搜查箱子”。

同一目标的辅助步骤可以一起描述，例如“给门锁刮痕拍照，放置比例尺并标注位置”。若输入含多个独立目标、指向不明或缺少已知条件，系统会请你修改；**澄清不消耗行动压力**。需要冒险时会先出现检定确认，确认后由系统掷骰；实际检定失败仍消耗行动，取消尚未执行的检定不消耗。

**移动与行动怎样计数？** 调查、取证、交涉和休整通常消耗1次有效行动；新路线或危险移动也会消耗。沿熟悉、安全的已知路线单独移动可以不增加压力，但仍需单独提交，不能顺便调查。危险中不能靠移动指令直接跳过围堵。

**右侧栏怎么用？**

- **世界观与玩家设定**：回看背景和角色资料；这些不是调查行动。
- **重要地点**：列出已发现的区域，“你在此”标明当前位置。查看地点资料不等于移动；请在输入框明确提交目的地。
- **调查证据**：“已发现”只表示知道线索存在；“已保全”才可支持最终证明。展开**详细信息**可以免费查看各组件进度、已知地点或人物条件、当前阻碍和下一步；尚未发现的来源不会提前揭露。点击其中的行动按钮会立即提交对应行动，并非免费取得证据；移动、合作和取证仍分别结算。
- **玩家状态**：查看自己的HP、SAN，以及敷料、休整次数和暂时压力。**NPC状态**显示可观察的身体或情绪，不公开其精确数值；离开视野的信息可能只是上次见面时的状态。
- **重要NPC与物品仓库**：回看已知人物资料和物品状态；林晚的证词取决于能否联系本人及其合作意愿，不绑定某个房间，已有副本可以随身处理。
- **玩法帮助**：需要时随时重读简要规则，不消耗行动。旁边的编辑按钮用于手动修改资料，不是角色在故事里获得物品或调查结果的方式。

新试炼以 **约24–30次有意义的行动** 为目标。发车压力到26时进入终局，最多再处理三次危机行动，然后作出最终决定。关键事件随章节出现，提示不是立即判定结果；你有机会回应。地点栏会逐步加入已发现区域，并标明当前位置。

顶部的**发车压力**是进度计数，不是消息条数；骰子确认不另算一次行动。**真相0/4**统计已由完整证据组合支持的结论，不是发现线索的数量。遭遇异常会进行SAN检定，损失取决于检定结果；心理压力会影响相关观察或集中注意的检定。
  ...
  scheduledEvents: [
    {
      ...
      branches: {
        foreground: {
          ...
          playerOptions: ['A. 留在原地，辨认广播和落锁声的来源', 'B. 询问乘务员封锁站台的真实原因', 'C. 趁通道尚未完全封死，观察站台方向', 'D. 自由行动'],
    ...
    },
    {
      ...
      branches: {
        present: {
          ...
          playerOptions: ['A. 立刻扶住许薇并保护她手里的钥匙', 'B. 扑向手提箱响动的位置，阻止黑暗中的人', 'C. 贴墙守住退路，辨认脚步和呼吸声', 'D. 自由行动'],
        ...
        remote: {
          ...
          playerOptions: ['A. 立即朝列车传来撞击声的方向赶去', 'B. 留在原地寻找照明并观察停电范围', 'C. 先保护手中的资料，再呼喊确认其他人的位置', 'D. 自由行动'],
    ...
    },
    {
      ...
      branches: {
        present: { outcome: 'painting_witnessed', instruction: '苏棠在玩家面前补完画作，第七个身影在最后一笔后出现。只呈现可观察异常，并让玩家决定如何询问或检查。', playerCue: '苏棠落下最后一笔时，湿画布上原本空着的位置缓慢显出第七道人影；她手中的画笔当场跌落。', playerOptions: ['A. 稳住苏棠，追问第七道人影是何时出现的', 'B. 不触碰画布，仔细检查颜料与笔触变化', 'C. 取出乘客名册，当场与画中人物逐一比对', 'D. 自由行动'] },
        nearby: { outcome: 'painting_intercept', instruction: '玩家在相邻区域听见苏棠惊呼，可以赶到候车厅查看刚完成的画作。', playerCue: '候车厅方向突然传来苏棠压抑不住的惊呼，紧接着是画笔落地的脆响。', playerOptions: ['A. 立即赶进候车厅查看苏棠是否安全', 'B. 先观察候车厅出口，防止有人趁乱离开', 'C. 呼喊苏棠回应，同时准备记录现场变化', 'D. 自由行动'] },
        expired: { outcome: 'painting_completed_offscreen', playerCue: '候车厅方向传来一声短促惊呼，随后彻底安静下来；那里的某件事已经发生。', playerOptions: ['A. 立即去候车厅查看那声惊呼', 'B. 先从门外观察是否有人离开候车厅', 'C. 记下声音发生的时刻，稍后调查现场', 'D. 自由行动'], leavesAftermath: true, aftermathLocationId: 'loc_005', aftermathInstruction: '玩家进入候车厅后发现苏棠已经完成画作；画上异常仍可调查。', aftermathPlayerCue: '候车厅里的画已经完成，湿润颜料间多出一道不属于任何已知乘客的第七个人影。' },
      },
    },
    {
      ...
      branches: {
        foreground: {
          ...
          playerOptions: ['A. 趁门锁复位，立即进入站务办公室', 'B. 先检查供电为何会自行恢复', 'C. 观察谁对办公室重新开放表现得异常紧张', 'D. 自由行动'],
    ...
    },
    {
      ...
      branches: {
        present: {
          ...
          playerOptions: ['A. 立刻扑灭火焰并抢下调度记录', 'B. 用相机先拍下记录内容和程岳的动作', 'C. 堵住出口，逼程岳说明为什么销毁文件', 'D. 自由行动'],
        ...
        nearby: {
          ...
          playerOptions: ['A. 立即冲进站务办公室阻止烧毁文件', 'B. 从门外观察并记录里面的人和动作', 'C. 呼叫附近的人封住办公室另一处出口', 'D. 自由行动'],
        ...
        absent: {
          ...
          playerOptions: ['A. 立即赶往站务办公室检查火光来源', 'B. 观察窗户和出口，寻找刚离开的人', 'C. 记录烟味与火光出现的时间，先保护现有证据', 'D. 自由行动'],
    ...
    },
    {
      ...
      branches: {
        present: {
          ...
          playerOptions: ['A. 明确承诺保护她，并提出安全保存证词和检修图', 'B. 先说明自己已经掌握的矛盾，争取她的信任', 'C. 不作承诺，追问地下入口与顾言之死的关系', 'D. 自由行动'],
        ...
        nearby: {
          ...
          playerOptions: ['A. 跟林晚去她认为安全的地方私下谈', 'B. 先确认有没有人跟踪她，再决定去向', 'C. 要她先说出一条可以验证的信息', 'D. 自由行动'],
        ...
        expired: {
          ...
          playerOptions: ['A. 立即去休息室寻找林晚留下的线索', 'B. 追踪林晚离开的方向', 'C. 先确认周围是否有人故意逼她离开', 'D. 自由行动'],
    ...
    },
    {
      ...
      branches: {
        foreground: {
          ...
          playerOptions: ['A. 检查铁门和积水痕迹，确认入口是否安全', 'B. 找林晚询问地下通道的结构与风险', 'C. 先准备照明和绳索，再进入地下', 'D. 自由行动'],
    ...
    },
    {
      ...
      branches: {
        foreground: {
          ...
          playerOptions: ['A. 把证据藏牢，利用两人的分歧争取脱身', 'B. 假意答应其中一人，逼他说出另一人的秘密', 'C. 寻找突破口，带着证据强行撤离', 'D. 自由行动'],
          npcUpdates: {
